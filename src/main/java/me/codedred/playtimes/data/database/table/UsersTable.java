...
public class UsersTable {
    ...
    public void createTable() {
        ...
        String createTableSql = String.format("CREATE TABLE IF NOT EXISTS `%s` (" + "`uniqueId` VARCHAR(36) NOT NULL,"
                + "`serverId` VARCHAR(255) NOT NULL," + "`playtime` BIGINT NOT NULL DEFAULT 0,"
                + "`afktime` BIGINT NOT NULL DEFAULT 0,"
                + "`lastUpdated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,"
                + "PRIMARY KEY (`uniqueId`, `serverId`))", TABLE_NAME);
    ...
    public void insertOrUpdate(String uuid, String serverId, long playtime, long afktime) {
        ...
        Async.run(() -> {
            ...
            String query = String
                    .format("INSERT INTO `%s` (`uniqueId`, `serverId`, `playtime`, `afktime`) VALUES (?, ?, ?, ?) "
                            + "ON DUPLICATE KEY UPDATE `playtime` = ?, `afktime` = ?", TABLE_NAME);
    ...
    public Map<String, Map<String, Long>> getPlaytimesByUuid(UUID uuid) {
        ...
        try (Connection conn = dataSource.getConnection();
                PreparedStatement preparedStatement = conn
                        .prepareStatement(String.format("SELECT * FROM `%s` WHERE `uniqueId` = ?", TABLE_NAME)))
        {
    ...
    /**
     * Deletes rows from the database where the last update was more than the
     * specified number of months ago.
     *
    ...
    public void purgeOldPlaytimes(String serverId, int months) {
        ...
        Async.run(() -> {
                    ...
                    PreparedStatement preparedStatement = conn.prepareStatement(query))
            {
                ...
                Bukkit.getLogger().info("[PlayTimes] Deleted " + deletedRows + " rows for serverId " + serverId

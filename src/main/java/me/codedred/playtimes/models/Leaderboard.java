...
public class Leaderboard {
    ...
    private static final Map<String, Integer> cachedLeaderboard = java.util.Collections
            .synchronizedMap(new LinkedHashMap<>());
    private static long lastCacheUpdate = 0;
    private static final long CACHE_DURATION = 30000; // 30 seconds

    /**
     * Grabs the uuids from the data file, sorts them, then removes redundant uuids
     * and updates the top 10
     *
    ...
    private Map<String, Integer> generateTopTen() {
        ...
        for (String key : data.getData().getConfigurationSection(LEADERBOARD).getKeys(false)) {
            ...
            allUsers.put(key, Integer.valueOf(data.getData().getString(LEADERBOARD + "." + key)));
    ...
    private Map<String, Integer> updateAllUserTimes(Map<String, Integer> allUsers) {
        ...
        for (Map.Entry<String, Integer> entry : allUsers.entrySet()) {
            ...
            int latestTime = (int) statManager.getPlayerStat(UUID.fromString(uuid), StatisticType.PLAYTIME);

            if (!dataManager.getConfig().getBoolean("top-playtime.track-rawtime", false)
                    && dataManager.hasAfkEnabled())
            {

                latestTime -= AFKManager.getInstance().getAFKTime(UUID.fromString(uuid)) * 20;

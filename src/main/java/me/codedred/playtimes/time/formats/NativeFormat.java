...
public class NativeFormat implements Timings {
    ...
    public String buildFormat(long time) {
        if (time < 60) return formatSeconds(time);

        int minutes = (int) (time / 60);
        int secondsLeft = (int) (time % 60);

        // Time less than 1 hour
        if (minutes < 60) return formatMinutes(minutes, secondsLeft);
    ...
    private String formatMinutes(int minutes, int secondsLeft) {
        ...
        if (secondsLeft > 0) format += formatSeconds(secondsLeft);
        return format;
    }
    ...
    private String formatHours(int hours, int minutesLeft, int secondsLeft) {
        ...
        if (minutesLeft >= 1) format += formatMinutes(minutesLeft, secondsLeft);
        else if (secondsLeft > 0) format += formatSeconds(secondsLeft);
        return format;
    }
